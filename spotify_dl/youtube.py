...
def download_songs(songs, download_directory, format_string, skip_mp3,
                   keep_playlist_order=False, no_overwrite=False, file_name_f=default_filename):
    """
    ...
    :param no_overwrite: Whether we should not download the song if it already exists
    ...
    if no_overwrite and skip_mp3:
        raise Exception("It is useless to avoid overwriting if you skip mp3 already")
    for song in songs:
