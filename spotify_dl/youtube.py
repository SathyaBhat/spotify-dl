...
def fetch_youtube_url(search_term, dev_key=None, scrape=False):
    ...
    if scrape or not dev_key:
        ...
        try:
            response = html.fromstring(requests.get(YOUTUBE_SEARCH_BASE + search_term).content)
            video = response.xpath("//a[contains(@class, 'yt-uix-tile-link')]/@href")
            video_id = re.search("((\?v=)[a-zA-Z0-9_-]{4,15})", video[0]).group(0)[3:]
            print(video_id)
            return YOUTUBE_VIDEO_URL + video_id
        except:
            print("Could not perform scrape search")
            return None
    else:
